...
public class TestAhoCorasick extends TestCase {
	...
	tree.prepare();
	SearchResult result = 
	    tree.startSearch("washington cut the apple tree".toCharArray());
	...
	SearchResult firstResult = 
	    tree.startSearch("johnjane".toCharArray());
	...
	assertEquals(new HashSet<Object>(Arrays.asList(new String[] {"xx", "x"})),
		     result.getOutputs());
	...
	assertEquals(new HashSet<Object>(Arrays.asList(new String[] {"one", "ne"})),
		     r.getOutputs());
	...
	assertTrue(iter.hasNext());
	r = (SearchResult) iter.next();
	assertEquals(new HashSet<Object>(Arrays.asList(new String[] {"moo"})),
		     r.getOutputs());
